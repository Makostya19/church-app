...
const Resources = () => {
  ...
  useEffect(() => { fetchResources(); }, [page, type, sortBy, order]);
  ...
  return (
    <div>
      <div style={{ background: 'linear-gradient(135deg, #1a1a2e, #0f3460)', padding: '4rem 2rem', textAlign: 'center', color: 'white' }}>
        <h1 style={{ fontSize: '2.5rem', fontWeight: '800', marginBottom: '0.5rem' }}>📚 Resources</h1>
        <p style={{ color: '#94a3b8', fontSize: '1.1rem' }}>Explore our church resources and materials</p>
      </div>
      ...
      <div style={{ maxWidth: '1100px', margin: '2rem auto', padding: '0 2rem' }}>
        <div style={{ background: 'white', borderRadius: '12px', padding: '1.5rem', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', marginBottom: '2rem' }}>
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
            ...
            <button onClick={fetchResources} style={{ padding: '0.75rem 1.5rem', background: '#2563eb', color: 'white', border: 'none', borderRadius: '8px', fontWeight: '600', cursor: 'pointer' }}>Search</button>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1.5rem' }}>
            {resources.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '3rem', background: 'white', borderRadius: '16px', gridColumn: '1/-1' }}>
                ...
                <p style={{ color: '#94a3b8' }}>No resources found</p>
              </div>
              ...
              <Link to={`/resources/${resource.id}`} key={resource.id} style={{ textDecoration: 'none' }}>
                <div style={{ background: 'white', borderRadius: '12px', padding: '1.5rem', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', transition: 'transform 0.2s, box-shadow 0.2s' }}
                  ...
                  <div style={{ width: '48px', height: '48px', background: '#eff6ff', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem', marginBottom: '1rem' }}>
                    {typeIcons[resource.type] || '📦'}
                  </div>
                  <span style={{ background: '#f0fdf4', color: '#16a34a', padding: '0.25rem 0.75rem', borderRadius: '20px', fontSize: '0.8rem', fontWeight: '600' }}>{resource.type}</span>
                  <h3 style={{ margin: '0.75rem 0 0.5rem', fontSize: '1.1rem', fontWeight: '700', color: '#1a1a2e' }}>{resource.title}</h3>
                  {resource.description && <p style={{ color: '#64748b', fontSize: '0.9rem', lineHeight: '1.6' }}>{resource.description.slice(0, 80)}...</p>}
                </div>
              </Link>
            ))}
          </div>
