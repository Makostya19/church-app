...
const AdminResources = () => {
  ...
  const [form, setForm] = useState({ title: '', description: '', type: 'document', category: '', url: '' });
  const [confirmModal, setConfirmModal] = useState({ isOpen: false, id: null });

  const fetchResources = async () => {
    try {
      const res = await client.get('/admin/resources');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editItem) {
        await client.put(`/resources/${editItem.id}`, form);
      ...
      setShowForm(false);
      setEditItem(null);
      setForm({ title: '', description: '', type: 'document', category: '', url: '' });
      fetchResources();
  ...
  const handleEdit = (item) => {
    setEditItem(item);
    setForm({ title: item.title, description: item.description || '', type: item.type, category: item.category || '', url: item.url || '' });
    setShowForm(true);
  };

  const handleDelete = async () => {
    try {
      await client.delete(`/resources/${confirmModal.id}`);
  ...
  const handleStatus = async (id, status) => {
    try {
      await client.patch(`/resources/${id}/status`, { status });
    ...
    <div>
      <div style={{ background: 'linear-gradient(135deg, #1a1a2e, #0f3460)', padding: '3rem 2rem', color: 'white' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={() => { setShowForm(!showForm); setEditItem(null); setForm({ title: '', description: '', type: 'document', category: '', url: '' }); }}
        ...
        {showForm && (
          <div style={{ background: 'white', borderRadius: '16px', padding: '2rem', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', marginBottom: '2rem' }}>
            <h3 style={{ fontSize: '1.2rem', fontWeight: '700', color: '#1a1a2e', marginBottom: '1.5rem' }}>
              {editItem ? 'Edit Resource' : 'Create Resource'}
            </h3>
            <form onSubmit={handleSubmit}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                ...
                <div>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '0.4rem', fontSize: '0.9rem' }}>Type</label>
                ...
                <div>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '0.4rem', fontSize: '0.9rem' }}>URL</label>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f8fafc' }}>
                {['Title', 'Type', 'Category', 'Status', 'Actions'].map(h => (
            ...
            <tbody>
              {resources.map((r, i) => (
                <tr key={r.id} style={{ borderBottom: '1px solid #f1f5f9', background: i % 2 === 0 ? 'white' : '#fafafa' }}>
                  <td style={{ padding: '1rem 1.25rem', fontWeight: '600', color: '#1a1a2e' }}>{r.title}</td>
                  <td style={{ padding: '1rem 1.25rem' }}>
                    <span style={{ background: '#eff6ff', color: '#2563eb', padding: '0.25rem 0.75rem', borderRadius: '20px', fontSize: '0.8rem', fontWeight: '600' }}>{r.type}</span>
                  </td>
                  <td style={{ padding: '1rem 1.25rem', color: '#64748b' }}>{r.category || '-'}</td>
                  <td style={{ padding: '1rem 1.25rem' }}>
                  ...
                  <td style={{ padding: '1rem 1.25rem' }}>
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      <button onClick={() => handleEdit(r)} style={{ padding: '0.4rem 0.9rem', background: '#eff6ff', color: '#2563eb', border: 'none', borderRadius: '8px', fontWeight: '600', fontSize: '0.85rem', cursor: 'pointer' }}>Edit</button>
                      <button onClick={() => setConfirmModal({ isOpen: true, id: r.id })} style={{ padding: '0.4rem 0.9rem', background: '#fee2e2', color: '#dc2626', border: 'none', borderRadius: '8px', fontWeight: '600', fontSize: '0.85rem', cursor: 'pointer' }}>Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
